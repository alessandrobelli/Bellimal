...
(function () {
  ...
  function navigate(slugOrCmd) {
    ...
    if (!slug) {
      printLine("usage: cd <slug>", { cls: "error" });
      return;
    }
    var match =
      nav.find(function (n) {
  ...
  var commands = {
    ...
    cat: function (args) {
      if (!args[0]) {
        printLine("usage: cat <slug>", { cls: "error" });
        return;
      }
      var arg = args[0];
      ...
      var preview = posts.find(function (p) {
        return p.slug === slug;
      });
      if (preview) {
    ...
    posts: function () {
      ...
      posts.forEach(function (p) {
        printLine(p.date + "  " + p.slug + " (" + p.readingTime + ")", {
          href: p.url,
        });
      });
    },
    ...
    sudo: function () {
      printLine("permission denied: nice try", { cls: "error" });
    },
    rm: function () {
      printLine("permission denied: nice try", { cls: "error" });
      var bongoDoge = [
      ...
      bongoDoge.forEach(function (line) {
        printLine(line);
      });
    },
    art: function () {
      ...
      asciiArt.forEach(function (line) {
        printLine(line);
      });
    },
  };
  ...
  function getCompletions(prefix) {
    ...
    pool = pool.filter(function (v, i, a) {
      return a.indexOf(v) === i;
    });
    if (!currentPrefix) return pool;
    return pool.filter(function (c) {
      return c.indexOf(currentPrefix) === 0;
    });
  }
  ...
  input.addEventListener("keydown", function (e) {
    if (e.key === "Enter") {
      ...
      if (matches.length === 1) {
        var sp = raw.indexOf(" ");
        input.value =
          sp === -1 ? matches[0] : raw.slice(0, sp + 1) + matches[0];
        setTimeout(function () {
          input.selectionStart = input.selectionEnd = input.value.length;
        }, 0);
